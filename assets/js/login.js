import {createUserWithEmailAndPassword,GoogleAuthProvider,sendEmailVerification,sendPasswordResetEmail,signInWithEmailAndPassword,signInWithPopup,updateProfile} from 'https://www.gstatic.com/firebasejs/11.0.2/firebase-auth.js';
...
$('#create-account').onclick=async()=>{try{show('Creating account…');const credential=await createUserWithEmailAndPassword(auth,$('#auth-email').value.trim(),$('#auth-password').value);const name=$('#auth-name').value.trim();if(name)await updateProfile(credential.user,{displayName:name});if(!credential.user.emailVerified)await sendEmailVerification(credential.user);await credential.user.getIdToken(true);show('Account created. We sent a verification email. Redirecting…');setTimeout(finish,700)}catch(e){show(e.message)}};
