...
let session=null,staff=[],products=[],orders=[],contentItems=[];
...
function renderContent(){const box=$('#admin-content');if(!box)return;box.innerHTML=contentItems.length?`<table class="data-table"><thead><tr><th>Type</th><th>Title</th><th>Description</th><th>Links</th><th>Published</th><th></th></tr></thead><tbody>${contentItems.map(item=>`<tr><td>${esc(item.type)}</td><td>${esc(item.title)}</td><td>${esc(item.description)}</td><td>${[item.mediaUrl,item.embedUrl,item.externalUrl,item.signupUrl].filter(Boolean).length}</td><td>${item.active!==false?'Yes':'No'}</td><td><button class="btn btn-ghost" data-edit-content="${esc(item.id)}">Edit</button></td></tr>`).join('')}</tbody></table>`:'<p>No site content has been added.</p>'}
async function loadStaffOrders(){const data=await apiFetch('getStaffOrders');orders=data.orders||[];renderOrders();renderMetrics()}
async function loadAdmin(){const [orderData,staffData,productData,contentData]=await Promise.all([apiFetch('getStaffOrders'),apiFetch('adminListStaff'),apiFetch('adminListProducts'),apiFetch('adminListContent')]);orders=orderData.orders||[];staff=staffData.users||[];products=productData.products||[];contentItems=contentData.items||[];renderOrders();renderMetrics();renderStaff();renderProducts();renderContent();$('#admin-panels').hidden=false}
async function load(){
...
document.addEventListener('click',async e=>{
  ...
  const contentEdit=e.target.closest('[data-edit-content]');if(contentEdit){const item=contentItems.find(x=>x.id===contentEdit.dataset.editContent);if(!item)return;const f=$('#content-form');for(const key of ['type','title','description','mediaUrl','embedUrl','externalUrl','signupUrl'])f.elements[key].value=item[key]??'';f.elements.active.checked=item.active!==false;f.dataset.id=item.id;f.scrollIntoView({behavior:'smooth'});}
});
...
$('#content-form')?.addEventListener('submit',async e=>{e.preventDefault();try{const f=new FormData(e.currentTarget);const item={id:e.currentTarget.dataset.id||'',type:f.get('type'),title:f.get('title'),description:f.get('description'),mediaUrl:f.get('mediaUrl'),embedUrl:f.get('embedUrl'),externalUrl:f.get('externalUrl'),signupUrl:f.get('signupUrl'),active:f.get('active')==='on'};await apiFetch('adminSaveContent',{method:'POST',body:{item}});delete e.currentTarget.dataset.id;e.currentTarget.reset();e.currentTarget.elements.active.checked=true;await loadAdmin()}catch(err){setError(err.message)}});
